...
import com.example.weatherapp.ui.favourites.FavouritesViewModel
...
class FavouriteAdapter(var favouriteList:ArrayList<WeatherResponse>,favouritesViewModel: FavouritesViewModel,context: Context):RecyclerView.Adapter<FavouriteAdapter.FavouriteViewHolder>() {
    ...
    override fun onBindViewHolder(holder: FavouriteViewHolder, position: Int) {
        ...
        Glide.with(holder.myView.imgIcon.context)
                .load(item.current.weather[0].icon?.let { getImage(it) })
                .placeholder(R.drawable.ic_cloudya)
                .into(holder.myView.imgIcon)
